...
On Error GoTo ErrorGrabar
    ...
    For i = 1 To LvListado.ListItems.Count - 1
        With VecCuentasNoUtilizadas(i)
            ...
            Select Case .Estado
            Case "A"
                Sql = "SpOcCuentasNoUtilizadasAgregar @C_CuentaContable='" & .Codigo & "'"
                Conec.Execute Sql
            Case "B"
                Sql = "SpOcCuentasNoUtilizadasBorrar @C_CuentaContable='" & .Codigo & "'"
                Conec.Execute Sql
            End Select
        End With
    Next
