...
Begin VB.Form A01_5B810 
   BackColor       =   &H00E0E0E0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Detalle Evolución Presupuestado/Real por Cuenta"
   ClientHeight    =   8055
   ClientLeft      =   -4710
   ClientTop       =   -1755
   ClientWidth     =   9765
   ...
   Begin DDSharpGrid2.SGGrid GridListado 
      ...
      StylesCollection=   $"A01_5B810.frx":0000
      ColumnsCollection=   $"A01_5B810.frx":1DD9
      ValueItems      =   $"A01_5B810.frx":35A4
   End
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   0
      Width           =   9690
      Begin VB.Label LbcuentaContable 
         AutoSize        =   -1  'True
         BackColor       =   &H00E0E0E0&
...
Public Sub Traer(FechaDesde As String, FechaHasta As String, Cuenta As String)
'On Error GoTo Errores
Dim Sql As String
Dim i As Integer
...
Dim Total As Double
    ...
    LbPeriodo.Caption = "Período: " & FechaDesde & " Hasta " & FechaHasta
    LbcuentaContable.Caption = "Cuenta Contable: " & BuscarDescCuentaContable(Cuenta)
    MousePointer = vbHourglass
    TbListado.CursorLocation = adUseClient
    Sql = "SpOcConsultaEvolucionCuentaContableDetalleRealTraer @PeriodoDesde ='" + Format(FechaDesde, "yyyy/MM") + _
    ...
    With TbListado
        ...
        While Not .EOF
            '        MONEDAK ImporteUss
            'Periodo C_Fecha  C_Concepto   C_Proveedor C_Emisor C_Empresa C_Sucu      C_Letra C_Numero
            GridListado.Array(i, 0) = !Periodo
            GridListado.Array(i, 1) = !C_Fecha
            'GridListado.Array(i, 2) = Format(ValN(!C_Sucu), "0000") & !C_Letra & Format(ValN(!C_Numero), "00000000")
            GridListado.Array(i, 2) = Trim(!C_Concepto)
            GridListado.Array(i, 3) = Format(ValN(!C_Importe), "#,##0.00")
             Total = Total + ValN(!C_Importe)
             i = i + 1
            .MoveNext
        Wend
        .Close
        GridListado.Array(i, 1) = "Total"
        'GridListado.Array(i, 2) = Format(TotPres, "#,##0.00")
        GridListado.Array(i, 3) = Format(Total, "#,##0.00")
...
Dim Columnas As Integer
    ...
    With ex
        ...
        .Range("A4").Select
        .ActiveCell.FormulaR1C1 = LbPeriodo.Caption
        .Range("A5").Select
        .ActiveCell.FormulaR1C1 = LbcuentaContable.Caption
