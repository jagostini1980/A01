...
Begin VB.Form A01_3400 
   BackColor       =   &H00E0E0E0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Emisión de Orden de Compra Especial"
   ClientHeight    =   8370
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12060
...
Public RsRenglones As ADODB.Recordset
  ...
  If ValidarEncabezado Then
    Conec.BeginTrans
     Sql = "SpOCOrdenesDeCompraCabeceraModificar @O_NumeroOrdenDeCompra=" + CStr(NroOrden) + _
           ...
           "', @U_Usuario = '" + Usuario + _
           "', @O_CentroDeCostoEmisor = " + CStr(VecCentroDeCostoEmisor(CmbCentroDeCostoEmisor.ListIndex).C_Codigo) & _
    ...
    For i = 1 To UBound(VecCentroCta)
      With VecCentroCta(i)
        ...
        Sql = "SpOCOrdenesDeCompraRenglonesAgregar @O_NumeroOrdenDeCompra = " & NroOrden & _
              ...
              ", @O_SinPresupuestar = " & IIf(.O_SinPresupuestar, 1, 0)
      End With
        Conec.Execute Sql
    Next
    Conec.CommitTrans
  ...
  If ValidarEncabezado Then
    ...
    Next
       ...
       Sql = "SpOCRecepcionOrdenesDeCompraCabeceraAgregar " & _
             ...
             ", @R_CodigoProveedor = " & VecProveedores(CmbProv.ListIndex).Codigo & _
             ", @R_LugarDeRecepcion = '" & CmbLugar.Text & _
             "', @U_Usuario ='" & Usuario & _
      ...
      With VecCentroCta(i)
        'If .CantRecibida > 0 Then
         ...
         Sql = "SpOCRecepcionOrdenesDeCompraRenglonesAgregar " & _
               ...
               "', @R_CantidadRecibida =" & Replace(.O_CantidadPedida, ",", ".") & _
               ", @R_PrecioRecepcionado = " & Precio & _
               ", @O_NumeroOrdenDeCompra = " & NroOrden & _
               ", @O_CentroDeCostoEmisor = '" & CentroEmisor & _
               "', @R_PrecioOrdenDeCompra = " & Precio
...
ErrorInsert:
    If Err.Number = 0 Then
       ...
       'Rta = MsgBox("La Orden de compra se Grabó correctamente con el Nº: " + CStr(NroOrden) + " ¿Desea imprimirla?", vbDefaultButton4)
       FrmMensaje.LbMensaje = "La Orden de compra se Grabó correctamente con el Nº: " + CStr(NroOrden) & _
                              Chr(13) & "La Recepción con el Nº: " & NroRecepcion & _
                              Chr(13) & " ¿Que desea hacer?"
       FrmMensaje.ChkImp1.Visible = True
       FrmMensaje.ChkImp2.Visible = True
       FrmMensaje.ChkImp1.Caption = "Imp. Orden de Compra"
       FrmMensaje.ChkImp2.Caption = "Imp. Autorización de carga contable"
       FrmMensaje.Show vbModal
       
       Modificado = False
       If FrmMensaje.Retorno = vbimprimir Then
          If FrmMensaje.Imp1 Then
             Call ConfImpresionDeOrden
             RepOrdenDeCompra.Show vbModal
          End If
          If FrmMensaje.Imp2 Then
             Call ConfImpresionDeAutorizacion(NroRecepcion, NroOrden)
...
Private Sub ConfImpresionDeAutorizacion(NroRecepcion As Long, NroOrden As Integer)
  Dim i As Integer
  Dim RsListado As New ADODB.Recordset
    
    RsListado.Fields.Append "Articulo", adVarChar, 150
    ...
    While i <= UBound(VecCentroCta)
        ...
        With VecCentroCta(i)
             RsListado!Articulo = BuscarDescArt(.O_CodigoArticulo)
             RsListado!Centro = .Centro_Descripcion & " - Cód. " & BuscarCodigoCentro(.O_CentroDeCosto)
             RsListado!CentroPadre = CmbCentroDeCostoEmisor.Text 'BuscarDescCentroEmisor(BuscarCentroPadre(.O_CentroDeCosto))
             RsListado!Cuenta = .Cta_Descripcion & " - Cód. " & .O_CuentaContable
             RsListado!Cantidad = .O_CantidadPedida
             RsListado!Precio = BuscarPrecio(.O_CodigoArticulo)
             RsListado!Importe = BuscarPrecio(.O_CodigoArticulo) * .O_CantidadPedida
             RsListado!NroOrden = Format(NroOrden, "00000000")
             RsListado!FechaOrden = CalFecha.Value
             RsListado!FormaDePago = CmbFormaDePago.Text
        End With
        i = i + 1
    Wend
    RsListado.MoveFirst
    RepAutorizacionDePago.TxtFecha = CStr(CalFecha.Value)
    RepAutorizacionDePago.TxtNroOrden.Text = Format(CStr(NroRecepcion), "000000000")
    RepAutorizacionDePago.TxtLugarDeEntrega = CmbLugar.Text
    RepAutorizacionDePago.TxtProv.Text = CmbProv.Text & " (Cod. " & VecProveedores(CmbProv.ListIndex).Codigo & ")"
    RepAutorizacionDePago.TxtUsuario.Text = TxtResp.Text
...
Private Sub CmdEliminar_Click()
    ...
    TotalMontoSinPres = TotalMontoSinPres - VecOrdenDeCompra(IndexBorrar).MontoSinPres
...
Private Sub CmdNueva_Click()
    ...
    Call BuscarCentroEmisor(CentroEmisor, CmbCentroDeCostoEmisor)
     ...
     Modificado = False
     CmdRequerimientos.Visible = TablaRequerimientos <> ""
     CmdRequerimientos.Enabled = TablaRequerimientos <> ""
...
Private Sub CmdRequerimientos_Click()
    ...
    If UBound(Requerimientos) Then
        ...
        For i = 1 To UBound(VecOrdenDeCompra)
            Total = Total + VecOrdenDeCompra(i).Cantidad * Precios(i)
        Next
        
        If Total > MaxSinAutorizacion Then
           MsgBox "La Orden de compra supera los $ " & MaxSinAutorizacion & ", no se puede hacer Como órden especial ya que requiere autorización"
           Exit Sub
        End If
        ...
        If UBound(Precios) = UBound(VecOrdenDeCompra) Then
            ...
            For R = 1 To UBound(VecOrdenDeCompra)
                LvListado.ListItems.Add
                VecOrdenDeCompra(R).PrecioUnit = Precios(R)
...
On Error GoTo Error
    ...
    For i = 1 To UBound(VecOrdenDeCompra)
      If i <> LvListado.SelectedItem.Index Then
         Total = Total + VecOrdenDeCompra(i).Cantidad * VecOrdenDeCompra(i).PrecioUnit
    ...
    If Val(TxtPrecioU.Text) * Val(TxtCant.Text) + Total > MaxSinAutorizacion Then
       MsgBox "La Orden de compra supera los $ " & MaxSinAutorizacion & ", no se puede hacer Como órden especial ya que requiere autorización"
       TxtPrecioU.SetFocus
       ValidarCargaOC = False
        Exit Function
