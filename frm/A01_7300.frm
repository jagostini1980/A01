...
Begin VB.Form A01_7300 
   ...
   Begin DDSharpGrid2.SGGrid GridListado 
      ...
      StylesCollection=   $"A01_7300.frx":0000
      ColumnsCollection=   "A01_7300.frx":1DD9
      ValueItems      =   $"A01_7300.frx":85C7
   End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin MSComCtl2.DTPicker CalDesde 
         ...
         CurrentDate     =   38972
      End
      Begin Controles.ComboEsp CmbProveedores 
         Height          =   315
         Left            =   5805
         TabIndex        =   7
         Top             =   180
...
MousePointer = vbHourglass
    CalDesde.Value = Date - 30
    CalHasta.Value = Date
    Call CargarCmbProveedoresPorCentroDeCosto(CmbProveedores, CentroEmisor)
...
Private Sub CmbProveedores_Click()
    If CmbProveedores.ListIndex = 0 Then
        GridListado.Columns(10).Hidden = False
        GridListado.Columns(6).Hidden = True
        GridListado.Columns(7).Hidden = True
        GridListado.Columns(8).Hidden = True
        GridListado.Columns(9).Hidden = True
    Else
        GridListado.Columns(10).Hidden = True
        GridListado.Columns(6).Hidden = False
        GridListado.Columns(7).Hidden = False
        GridListado.Columns(8).Hidden = False
        GridListado.Columns(9).Hidden = False
    End If
End Sub
...
Dim TotUsado As Double
    ...
    If CmbProveedores.ListIndex = 0 Then
             Sql = "Admin.ElPulquiAdministracion..SpProveConsultaFacturasPendientesPorProveedor @CentroDeCosto ='" & BuscarJerarquiaCentro(CentroEmisor) & _
                                                 "', @FecDesde ='" & CalDesde.Value & _
                                                 "', @FecHasta ='" & CalHasta.Value & "'"
         ...
         With TbListado
             ...
             While Not .EOF
                 ...
                 GridListado.Array(i, 2) = Replace(!Comprobante, " ", "0")
                 GridListado.Array(i, 3) = Format(ValN(!Importe), "#,##0.00")
                 GridListado.Array(i, 4) = Format(ValN(!SALDO), "#,##0.00")
                 GridListado.Array(i, 9) = BuscarDescProv(!C_PROVE)
                  i = i + 1
                 .MoveNext
             Wend
             .Close
         End With
    Else
         Sql = "Admin.ElPulquiAdministracion..SpProveConsultaFacturasPorProveedor @Prove ='" & Rellenar(Str(VecProveedoresPorCentroDeCosto(CmbProveedores.ListIndex).Codigo), 4) & _
                                                 "', @FecDesde ='" & CalDesde.Value & _
                                                 "', @FecHasta ='" & CalHasta.Value & "'"
         ...
         With TbListado
             ...
             While Not .EOF
                 ...
                 GridListado.Array(i, 3) = Format(ValN(!Importe), "#,##0.00")
                 GridListado.Array(i, 4) = Format(ValN(!SALDO), "#,##0.00")
                 GridListado.Array(i, 5) = ValN(!ORDENDEPAGO)
                 GridListado.Array(i, 6) = Format(!FECHADEPAGO, "dd/MM/yyyy")
                 GridListado.Array(i, 7) = VerificarNulo(!TIPOPAGO)
                 GridListado.Array(i, 8) = Format(ValN(!TOTALPAGO), "#,##0.00")
                  i = i + 1
                 .MoveNext
             Wend
             .Close
...
Dim Columnas As Integer
    ...
    With ex
        ...
        .Range("A4").Select
        .ActiveCell.FormulaR1C1 = "Fecha Desde: " & Format(CalDesde.Value, "dd/MM/yyyy") & " Hasta " & Format(CalHasta.Value, "dd/MM/yyyy")
        .Range("A5").Select
        .ActiveCell.FormulaR1C1 = "Proveedor: " & CmbProveedores.Text
