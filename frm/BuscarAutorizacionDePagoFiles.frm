...
Begin VB.Form BuscarAutorizacionDePagoFiles 
   BackColor       =   &H00E0E0E0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Buscar Autorización de Pago Para Files"
   ClientHeight    =   6915
   ClientLeft      =   3225
   ClientTop       =   2250
   ClientWidth     =   11730
   ...
   End
End
Attribute VB_Name = "BuscarAutorizacionDePagoFiles"
...
Private Sub CargarLV(FechaDesde As Date, FechaHasta As Date)
   ...
   Sql = "SpOCAutorizacionesDePagoCabeceraFilesTraer @FechaDesde =" & FechaSQL(CStr(FechaDesde), "SQL") & _
   ...
   If .RecordCount > 0 Then
      For i = 1 To .RecordCount
        If ValN(!A_FileNro) > 0 Or ValN(!A_Paquete) > 0 Then
            LvListado.ListItems.Add
            LvListado.ListItems(LvListado.ListItems.Count).Text = Format(!A_NumeroDeAutorizacionDePago, "0000000000")
            LvListado.ListItems(LvListado.ListItems.Count).SubItems(1) = !A_Fecha
            LvListado.ListItems(LvListado.ListItems.Count).SubItems(2) = BuscarDescProv(!A_CodigoProveedor)
            LvListado.ListItems(LvListado.ListItems.Count).SubItems(3) = VerificarNulo(!A_FileNro)
            LvListado.ListItems(LvListado.ListItems.Count).SubItems(4) = BuscarDescPaquete(ValN(!A_Paquete)) & IIf(Not IsNull(!A_Desde), " - Desde " & !A_Desde & " Hasta " & !A_Hasta, "")
            LvListado.ListItems(LvListado.ListItems.Count).SubItems(5) = IIf(IsNull(!A_FechaAnulacion), "", Format(!A_FechaAnulacion, "dd/MMM/yyyy HH:mm"))
            LvListado.ListItems(LvListado.ListItems.Count).SubItems(6) = !A_Observaciones
        End If
        .MoveNext
      Next
    End If
...
Private Sub CrearEncabezado()
    ...
    LvListado.ColumnHeaders.Add , , "Proveedor", 2250
    LvListado.ColumnHeaders.Add , , "File Nº", 1000
    LvListado.ColumnHeaders.Add , , "Destino", 2000
    LvListado.ColumnHeaders.Add , , "F. de Anulación", 1400
    LvListado.ColumnHeaders.Add , , "Observaciones", LvListado.Width - 9500
End Sub
...
Private Sub LVListado_DblClick()
    A01_4600.NroAutorizacion = Val(LvListado.SelectedItem.Text)
    Unload Me
End Sub
