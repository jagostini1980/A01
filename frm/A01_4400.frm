...
Begin VB.Form A01_4400 
   BackColor       =   &H00E0E0E0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Certificación de Servicios Especial"
   ClientHeight    =   6555
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10440
...
On Error GoTo Errores
    ...
    If CmbCentrosDeCostos.ListIndex <= 0 Then
        MsgBox "Debe seleccionar un Centro de Costo", vbInformation
        CmbCentrosDeCostos.SetFocus
        ValidarCarga = False
        Exit Function
    End If
    ...
    If CmbCuentas.ListIndex <= 0 Then
        MsgBox "Debe seleccionar una Cuenta Contable", vbInformation
        CmbCuentas.SetFocus
        ValidarCarga = False
        Exit Function
    End If
    ...
    Else
         ...
         If MontoDisponible < Val(TxtPrecioU.Text) Then
             ...
             If Rta = vbNo Then
                 ...
                 Exit Function
             End If
             'si no alcanza con lo presupuestado para una cuenta
    ...
    If Val(TxtPrecioU.Text) + Total > MaxSinAutorizacion Then
       MsgBox "La Orden de Contratación supera los $ " & MaxSinAutorizacion & ", no se puede hacer Como órden especial ya que requiere autorización"
       TxtPrecioU.SetFocus
       ValidarCarga = False
        Exit Function
    End If
...
Private Sub CMDBuscar_Click()
    Unload BuscarAutorizacionDePagoEspecial
    BuscarAutorizacionDePagoEspecial.Show vbModal
...
On Error GoTo Error
 ...
 With RsCargar
    ...
    If VerificarNulo(!A_FondoFijoNro, "N") <> 0 Then
        MsgBox "La Autorización de Pago es de F. F."
        Call CmdNuevo_Click
        Exit Sub
    End If
    ...
    End If
     ...
     CmbProv.Enabled = False
    Call UbicarEmpresa(VerificarNulo(!A_Empresa), CmbEmp)
  ...
  If ValidarEncabezado Then
    Conec.BeginTrans
       Sql = "SpOCAutorizacionesDePagoCabeceraAgregar" & _
                    ...
                    " , @A_Empresa='" & VecEmpresas(CmbEmp.ListIndex).Codigo & "'"
    ...
    If Err.Number = 0 Then
      ...
      'Rta = MsgBox("La Autorización de Pago se Grabó correctamente con el Nº: " & CStr(NroAutorizacion) & " ¿Desea imprimirla la Autorización?", vbYesNo)
       ...
       If FrmMensaje.Retorno = vbimprimir Then
         ...
         RepAutorizacionDePagoServicioEspecial.Show vbModal
       End If
...
On Error GoTo Error
  ...
  If MenuEmisionOrdenCompra.Cuadros.Filename <> "" Then
         Call ConfImpresionDeAutorizacion
         RepAutorizacionDePagoServicioEspecial.Run
        'guarda la orden de compra como un PDF
         ...
         myPDFExport.Export RepAutorizacionDePagoServicioEspecial.Pages
         Unload RepAutorizacionDePagoServicioEspecial
  End If
Error:
    If Err.Number = 0 Then
...
Private Sub CmdImprimir_Click()
    Call ConfImpresionDeAutorizacion
    RepAutorizacionDePagoServicioEspecial.Show
End Sub
...
Private Sub ConfImpresionDeAutorizacion()
  ...
  Set RsListado = New ADODB.Recordset
    ...
    RepAutorizacionDePagoServicioEspecial.TxtFecha = Format(CalFecha.Value, "MM/yyyy")
    RepAutorizacionDePagoServicioEspecial.TxtFEmision = CStr(CalFechaEmitida.Value)
    RepAutorizacionDePagoServicioEspecial.TxtNroOrden.Text = TxtNroAutorizacion.Text
    RepAutorizacionDePagoServicioEspecial.BarNroAutorizacion.Caption = TxtNroAutorizacion.Text
    RepAutorizacionDePagoServicioEspecial.TxtProv.Text = CmbProv.Text & " (Cód. " & VecProveedores(CmbProv.ListIndex).Codigo & ")"
    RepAutorizacionDePagoServicioEspecial.TxtAnulada.Visible = LBAnulada.Visible
    RepAutorizacionDePagoServicioEspecial.TxtAnulada.Text = LBAnulada.Caption
    RepAutorizacionDePagoServicioEspecial.TxtObservaciones.Text = TxtObs.Text
    RepAutorizacionDePagoServicioEspecial.Zoom = -1
    RepAutorizacionDePagoServicioEspecial.DataControl1.Recordset = RsListado
End Sub
