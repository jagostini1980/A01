...
Begin VB.Form A01_5B120 
   BackColor       =   &H00E0E0E0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Detalle de Gastor"
   ClientHeight    =   8055
   ClientLeft      =   -4710
   ClientTop       =   -1755
   ClientWidth     =   9900
   ...
   Begin DDSharpGrid2.SGGrid GridListado 
      ...
      StylesCollection=   $"A01_5B120.frx":0000
      ColumnsCollection=   "A01_5B120.frx":1DD9
      ValueItems      =   $"A01_5B120.frx":69A3
   End
...
Dim TotUsado As Double
    ...
    Sql = "SpOcConsultaEstadoDePresuestoDetalleDeGastos @CentroEmisor ='" & VecCentroDeCostoEmisor(CmbCentroDeCostoEmisor.ListIndex).C_Codigo & _
                                                    "', @Periodo='" & Format(CalPeriodo, "MM/yyyy") & _
    ...
    With TbListado
        ...
        While Not .EOF
            GridListado.Array(i, 0) = !Tipo
            GridListado.Array(i, 1) = !O_NumeroOrdenDeCompra
            GridListado.Array(i, 2) = !O_Fecha
            GridListado.Array(i, 3) = Format(ValN(!Monto), "#,##0.00")
            GridListado.Array(i, 4) = Format(ValN(!SinPres), "#,##0.00")
            GridListado.Array(i, 5) = Format(ValN(!Monto) + ValN(!SinPres), "#,##0.00")
            GridListado.Array(i, 6) = !U_Usuario
            TotUsado = TotUsado + ValN(!Monto)
             i = i + 1
            .MoveNext
        Wend
        .Close
...
Dim Columnas As Integer
    ...
    With ex
        ...
        .Range("C5").Select
        .ActiveCell.FormulaR1C1 = "Cuenta Contable: " & TxtCuentaContable.Text
